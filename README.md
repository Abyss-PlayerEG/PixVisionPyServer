...
PixVisionPyServer 是基于 **Python FastAPI** 构建的辅助服务端，为 PixVision 主项目提供两大核心能力：
...
| AI 服务 | OpenAI SDK | 兼容 OpenAI 协议的大模型调用 |
| 运行环境 | Python 3.14+ | — |
...
|--------|------|
| 0 | 成功 |
| 400 | 请求参数错误（无效的平台、用户 ID 格式等） |
| 404 | 用户不存在 |
| 500 | 服务器内部错误 |
| 502 | 上游 API 请求失败（限流、网络错误等） |
| 503 | AI 服务未配置或不可用 |
...
- [AI 文案审核接口](doc/AI文案审核接口.md)
- [B 站账号检测 API 集成文档](doc/B站账号检测%20API%20集成文档.md)

---

**PixVisionPyServer** — 像素视觉 Python FastAPI 辅助服务端
